		...
		BEGIN
			...
			BEGIN
				...
				SELECT TOP 1 @DeviceTypeID = 0
					...
					,@DeviceConditionID = dc.DeviceConditionID
					,@DeviceVendorID = dc.DeviceVendorID
				FROM Devices d(NOLOCK)
				JOIN (
					SELECT TOP 1 dc2.*
					FROM DeviceXCarrier dc2(NOLOCK)
					JOIN Devices d2(NOLOCK) ON d2.DeviceID = dc2.DeviceID
					WHERE dc2.DeviceID = @DeviceID
						AND dc2.CarrierID = @CarrierID
						AND (
								(
									@CarrierID IN (
										122
								...
								OR (
									@CarrierID = 178
									AND d2.Make = 'Apple'
									AND dc2.DeviceVendorID = 600
									)
								OR NOT EXISTS (
									SELECT 1
									FROM DeviceXCarrier dc2 (NOLOCK)
									WHERE dc2.DeviceID = d2.DeviceID
										AND dc2.CarrierID = @CarrierID
										AND dc2.DeviceVendorID IN (
											100
											,600
											)
									)
					...
					ORDER BY CASE 
							...
							END
						,dc2.Margin DESC
						,dc2.AddedDateTime ASC
					) AS dc ON dc.DeviceID = d.DeviceID			-- Device Vendor requirements
				...
				IF ISNULL(@DeviceXCarrierID, 0) = 0
					BEGIN
							SET @DeviceConditionID = 100

							SELECT TOP 1 @DeviceVendorID = dxc.DeviceVendorID,
									@DeviceXCarrierID = dxc.DeviceXCarrierID
				...
				SET LineStatusMasterID = @LineStatusMasterID
					...
					,ReplenishmentCCAID = @CustomerXCarrierAccountsID	-- replenishment flag
				...
				-- CONFIGURE Device to vCom Solutions Service Provider if not configured
				IF NOT EXISTS (
					SELECT TOP 1 1
					FROM CustomerXProductCatalog (NOLOCK)
					WHERE CustomerID = @vComSPCustomerID
						AND CarrierID = @CarrierID
						AND CategoryPlanID = @DeviceID
						AND ProductCatalogCategoryMasterID = 5		-- Device
						AND StatusID = 1
				)
				BEGIN
					-- Configure device for vCom Service Provider
					INSERT INTO CustomerXProductCatalog (
						CustomerID
						,CategoryPlanID
						,ProductCatalogCategoryMasterID
						,CarrierID
				...
				BEGIN
					...
					SELECT TOP 1 @MobilityOrderID
						,'Inventory Management for ' + customer_name
